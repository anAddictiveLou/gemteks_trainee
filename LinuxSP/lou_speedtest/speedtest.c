...
int get_best_server(server_data_t *nearest_servers) {
    ...
    for(i=0; i<NEAREST_SERVERS_NUM; i++) {
        ...
        /* by lou */
        //nearest_servers[i].rtt = get_rtt(nearest_servers[i].domain_name);
    ...
    /* by lou */
    // int best_index = 0;
    // for (int i = 0; i < NEAREST_SERVERS_NUM - 1; i++)
    // {
    //     if (nearest_servers[i+1].rtt < nearest_servers[i].rtt)
    //     {
    //         best_index = i+1;
    //     }
    // }
    // printf("best rtt = %Lf ", nearest_servers[best_index].rtt);
    return best_index;
}
...
int main() {
    ...
    /* get some i4 about host in the Internet */
    ...
    /* get list of nearest server for speedtesting */
